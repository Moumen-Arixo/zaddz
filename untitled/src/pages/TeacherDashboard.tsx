...
import { setupCourseFoldersInGDrive } from "../lib/gdrive";
...
export function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    fetchApi("/api/courses")
      .then(res => res.json())
      .then(data => {
        if (!data || data.error) throw new Error("API returned error");
  ...
  const handleCreateCourse = async (e: React.FormEvent) => {
    ...
    // Setup GDrive folders automatically
    let gDriveData = null;
    try {
      const gdToken = localStorage.getItem("gdrive_access_token");
      if (gdToken) {
         gDriveData = await setupCourseFoldersInGDrive(gdToken, newCourse.subject || 'منوعة', newCourse.title);
      }
    } catch (err) {
      console.error("GDrive folder setup failed", err);
    }
    ...
    try {
      ...
      try {
        ...
        // API might return 404 HTML on static host without backend
      ...
      alert("تم نشر الدورة وتجهيز مساحة التخزين بنجاح!");
       ...
       console.warn("DB Error, using local storage fallback", err);
       // Local storage fallback for frontend-only demo deployments
      ...
      // Reset form
      setNewCourse({ title: "", level: "bac", subject: "math", description: "", price: "", discount: "0", bgClass: "bg-gradient-to-br from-indigo-500/20 to-purple-500/20" });
      alert("تم نشر الدورة بنجاح! (تم الحفظ محلياً)");
    }
  };
