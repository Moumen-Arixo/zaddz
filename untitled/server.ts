...
// The user provided database connection string.
// Note: Free hosts like InfinityFree typically block external connections (e.g., from this cloud environment).
// The health endpoint will verify if the connection succeeds or times out.
const dbConfig = {
...
async function startServer() {
  ...
  app.put("/api/settings", async (req, res) => {
    try {
      ...
      const { copyrightYear, primaryColor, secondaryColor, borderRadius } = req.body;
      const newSettings = {
      ...
      try {
        ...
        `);
        // We use INSERT ON DUPLICATE KEY UPDATE but since it's standard let's just REPLACE or simple INSERT
  ...
  app.post("/api/courses", async (req, res) => {
    try {
      // Ensure courses table has subject column if we really need it, but let's just save title and level for now.
